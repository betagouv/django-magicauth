...
Note : We do not test that the javascript actually does the redirect. We just test the
django template's context.
"""
...
def test_wait_page_raises_404_if_unsafe_next_url(client):
    token = factories.MagicTokenFactory()
    response = open_magic_link_with_wait(client, token, "http://www.myfishingsite.com/")
